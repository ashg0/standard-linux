...
static void add_to_epoll_fd_list(int fd, uint32_t ep_events);
static void remove_from_epoll_fd_list(int fd);
...
setup_environment(char *root, char *user, char *group)
{
    ...
    if (!gr) {
        ...
        exit(1);
    }https://www.geekpage.jp/programming/linux-network/so_reuseaddr.php
...
listen_socket(char *port)
{
    ...
    for (ai = res; ai; ai = ai->ai_next) {
        ...
        if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int)) < 0)
            log_exit("failed setsockopt(SO_REUSEADDR)");
        ...
        for (int i = 0; i < epoll_ret; i++) {
            ...
            if (events[i].data.fd == server_fd) {
                ...
                for (;;) {
                    ...
                    add_to_epoll_fd_list(sock, EPOLLIN);
                }
            } else {
                printf("sock readable: %d\n", events[i].data.fd);
                FILE *inf = fdopen(events[i].data.fd, "r");
                FILE *outf = fdopen(events[i].data.fd, "w");
                service(inf, outf, docroot);
                remove_from_epoll_fd_list(events[i].data.fd);
                close(sock);
            }
        }
        printf("epoll_ret: %d\n", epoll_ret);
